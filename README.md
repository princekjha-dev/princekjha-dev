...
I build backend tools and AI pipelines — things that actually run, not just demo. Before starting college I worked as a Data Annotation Specialist at Cogito Tech, labeling 10,000+ images for CV pipelines. Since then I've shipped two open-source tools, built a JWT auth API with Spring Boot, and learned how async systems and RAG pipelines work in practice.
...
**[SecurePass API](https://github.com/princekjha-dev/securepass-api)** &nbsp; `in progress`

REST API for user authentication and role-based access — built while learning Java and Spring Boot.

- Stateless **JWT auth** with access/refresh token flow
- BCrypt hashing + PostgreSQL token blacklist for logout
- Role-gated endpoints via Spring Security filter chains
- Containerized with **Docker Compose** — one command setup
